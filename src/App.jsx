...
// Instructions are in README.md Step 1
const CLIENT_ID = "318030100403-j5cr46evcj4khs954l06p15j40uqtann.apps.googleusercontent.com";

const SHEET_ID  = "1_cR32Owor2_at23OvNnL9ETlPlJDbfKM7ispp9VvmWs";
const SHEET_GID = 781126454;
...
export default function App() {
  ...
  useEffect(() => {
    // Poll until GSI script is ready (it loads async)
    let attempts = 0;
    const poll = setInterval(() => {
      ...
      if (attempts > 30) clearInterval(poll); // give up after 6s
    }, 200);
  ...
  const handleSignIn = () => {
    if (!tokenClientRef.current) {
      setError("Google Sign-In not ready yet — wait a moment and try again");
      return;
    }
  ...
  const fetchSheetData = useCallback(async (accessToken) => {
    ...
    setError(null);
    try {
      // Step 1: get spreadsheet metadata to find sheet name by GID
      ...
      const sheetName = sheetMeta
        ? sheetMeta.properties.title
        : meta.sheets?.[0]?.properties?.title;
      if (!sheetName) throw new Error("Sheet tab not found");

      // Also get user email from token info
      try {
      ...
      // Step 2: get all values from the sheet
      const dataRes = await fetch(
        `https://sheets.googleapis.com/v4/spreadsheets/${SHEET_ID}/values/${encodeURIComponent(sheetName)}`,
        { headers: { Authorization: `Bearer ${accessToken}` } }
      );
      if (!dataRes.ok) throw new Error(`Could not read sheet: ${dataRes.status}`);
      const data = await dataRes.json();

      const parsed = parseRows(data.values || []);
      if (!parsed.length) throw new Error("No data rows found in sheet");
  ...
  if (!token && !loading) {
    return (
      <div style={{ minHeight: "100vh", background: "#0a0a0a", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 24, gap: 0 }}>
        <div style={{ fontWeight: 800, fontSize: 38, letterSpacing: 5, color: "#f5f5f5", marginBottom: 6 }}>
          PUSH<span style={{ color: ORANGE }}>DESK</span>
        </div>
        <div style={{ fontFamily: "monospace", fontSize: 11, color: "#333", letterSpacing: 3, textTransform: "uppercase", marginBottom: 48 }}>
          Weekly Push Command Center
        </div>

        <div style={{ background: "#111", border: "1px solid #1e1e1e", borderRadius: 10, padding: "32px 40px", display: "flex", flexDirection: "column", alignItems: "center", gap: 20, maxWidth: 400, width: "100%" }}>
          <div style={{ width: 48, height: 48, borderRadius: "50%", background: "#1a1a1a", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 24 }}>
            🔐
          </div>
          <div style={{ textAlign: "center" }}>
            <div style={{ fontSize: 15, fontWeight: 600, marginBottom: 6 }}>Sign in with your Jumia account</div>
            <div style={{ fontSize: 12, color: "#555", lineHeight: 1.6 }}>Uses your existing Google Workspace login — no password needed</div>
          </div>
          <button
            onClick={handleSignIn}
            style={{ width: "100%", padding: "12px 24px", background: ORANGE, border: "none", color: "#000", borderRadius: 6, fontWeight: 700, fontSize: 14, cursor: "pointer", letterSpacing: 0.5, display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}
          >
            <svg width="18" height="18" viewBox="0 0 18 18">
              <path fill="#000" d="M16.51 8H8.98v3h4.3c-.18 1-.74 1.48-1.6 2.04v2.01h2.6a7.8 7.8 0 0 0 2.38-5.88c0-.57-.05-.66-.15-1.18Z"/>
              <path fill="#333" d="M8.98 17c2.16 0 3.97-.72 5.3-1.94l-2.6-2a4.8 4.8 0 0 1-7.18-2.54H1.83v2.07A8 8 0 0 0 8.98 17Z"/>
              <path fill="#555" d="M4.5 10.52a4.8 4.8 0 0 1 0-3.04V5.41H1.83a8 8 0 0 0 0 7.18l2.67-2.07Z"/>
              <path fill="#222" d="M8.98 4.18c1.17 0 2.23.4 3.06 1.2l2.3-2.3A8 8 0 0 0 1.83 5.4L4.5 7.48a4.77 4.77 0 0 1 4.48-3.3Z"/>
            </svg>
            Sign in with Google
          </button>
          {error && (
            <div style={{ width: "100%", padding: "8px 12px", background: "#7f1d1d20", border: "1px solid #dc262640", borderRadius: 4, color: "#fca5a5", fontSize: 11, fontFamily: "monospace" }}>
              ⚠ {error}
            </div>
          )}
          <div style={{ fontSize: 11, color: "#333", textAlign: "center", lineHeight: 1.6 }}>
            Only accessible to <span style={{ color: "#555" }}>@jumia.com</span> accounts
          </div>
        </div>
      </div>
    );
  ...
  if (loading && !rows) {
    return (
      <div style={{ minHeight: "100vh", background: "#0a0a0a", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 16 }}>
        <div style={{ fontWeight: 800, fontSize: 28, letterSpacing: 5, color: "#f5f5f5" }}>PUSH<span style={{ color: ORANGE }}>DESK</span></div>
        <div style={{ fontFamily: "monospace", fontSize: 12, color: "#444" }}>Loading sheet data…</div>
      </div>
    );
  }
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0a0a0a", color: "#f5f5f5", fontFamily: "system-ui, sans-serif" }}>
      ...
      <div style={{ background: "#0f0f0f", borderBottom: "1px solid #1f1f1f", padding: "10px 20px", display: "flex", alignItems: "center", gap: 12, position: "sticky", top: 0, zIndex: 20 }}>
        <span style={{ fontWeight: 800, fontSize: 18, letterSpacing: 3 }}>PUSH<span style={{ color: ORANGE }}>DESK</span></span>
        ...
        <div style={{ marginLeft: "auto", display: "flex", gap: 8, alignItems: "center" }}>
          ...
          {userEmail && <span style={{ fontFamily: "monospace", fontSize: 10, color: "#333" }}>{userEmail}</span>}
          <button onClick={() => fetchSheetData(token)} disabled={loading} style={{ padding: "5px 12px", background: "transparent", border: "1px solid #2a2a2a", color: loading ? "#333" : "#666", borderRadius: 4, cursor: "pointer", fontSize: 12 }}>
            {loading ? "Syncing…" : "↻ Refresh"}
          </button>
          <select value={week} onChange={(e) => { setWeek(+e.target.value); setSlots(Array(TOTAL_SLOTS).fill(null)); }} style={sel}>
            {weeks.map((w) => <option key={w} value={w}>Week {w}</option>)}
          </select>
          <button onClick={() => { setToken(null); setRows(null); setUserEmail(null); }} style={{ padding: "5px 10px", background: "transparent", border: "1px solid #1a1a1a", color: "#333", borderRadius: 4, cursor: "pointer", fontSize: 11 }}>
            Sign out
          </button>
        </div>
      </div>
        ...
        {tab === "week" && (
          <>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
              ...
              {filtered.map((r) => <PushCard key={r.id} row={r} assigned={assignedSet.has(r.id)} onAssign={() => addToSlot(r)} onDragStart={() => setDragId(r.id)} />)}
            </div>
          </>
        )}
          ...
          <div style={{ display: "flex", gap: 16 }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ marginBottom: 16 }}>
                <div style={{ fontFamily: "monospace", fontSize: 10, color: "#333", letterSpacing: 1.5, textTransform: "uppercase", marginBottom: 8 }}>Slider</div>
              ...
              <div>
                <div style={{ fontFamily: "monospace", fontSize: 10, color: "#333", letterSpacing: 1.5, textTransform: "uppercase", marginBottom: 8 }}>Product floor</div>
